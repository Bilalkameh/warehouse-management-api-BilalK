...
public class TestFileStorageService : IFileStorageService
{
    ...
    public async Task UploadAsync(Stream fileStream, long fileSize, string objectKey, string contentType, CancellationToken cancellationToken)
    {
        using var memoryStream = new MemoryStream();

        await fileStream.CopyToAsync(memoryStream, cancellationToken);

        _files[objectKey] = new StoredFile(memoryStream.ToArray(), fileSize, contentType);
    }
    ...
    {
        return Task.FromResult($"https://test-storage.local/{objectKey}");
    }
    ...
    public record StoredFile(byte[] Content, long FileSize, string ContentType);
}